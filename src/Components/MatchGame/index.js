...
class MatchGame extends Component {
  ...
  onChangingTabItems = tabId => {
    this.setState({...defaultValues, activeTabId: tabId})
  }
  ...
  onClickThumbnail = id => {
    ...
    if (id === currentImageId) {
      ...
      const currentImageIds = imagesList[index].id
      const {score} = this.state
      this.setState({
        currentImageId: currentImageIds,
        score: score + 1,
        currentIndex: index,
      })
    }
  ...
  render() {
    const {timer, score, gameOver, activeTabId, currentIndex} = this.state
    ...
    return (
      <div className="bg-container">
        ...
        {gameOver ? (
          ...
          <GameInProgress
            imagesList={thumbnailImagesList}
            tabsList={tabsList}
            onClicking={this.onChangingTabItems}
